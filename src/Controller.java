...
public class Controller {
	...
	/** boolean variable to determine if this is a windows environment to
	 * determine if slashes should be switched */
	private boolean isWindowsApp;
			...
			for (int i = 0; i < movieFiles.length; i++) {//loop to handle adding the file data for each movie file
				if (movieFiles[i].isFile() && !movieFiles[i].isHidden()) {//make sure to only check actual files
					...
					if (movieFormats.contains(extension)) {//check if this format is in the movie list
						...
						if (checkExistence(new File(MOVIE_DATA_PATH + movie.getTitle()), true) && checkExistence(movieDataFile, false)) {
							//checks for the file path directories, and the checks for the file itself, creating them if necessary
							try {
								fileReader = new BufferedReader(new FileReader(movieDataFile));
								while (fileReader.ready()) {
									lastTag = parseDataLine(fileReader.readLine(), movie, lastTag);//parses the line and updates the movie
								}
	...
	private boolean checkExistence(File directory, boolean checkForDirectory){
		...
		try {
			if (checkForDirectory) {
				if (!directory.exists() && !directory.mkdir()) {//check if it exists; if not, make it; utilizes Java's shortcut boolean operations
					JOptionPane.showConfirmDialog(null, "There was an error creating " + directory.getCanonicalPath() + ".", "Error",
